...
For this experiment, we employ a high-performance cloud-based voiceprint recognition solution from iFlyTek that takes in the voice samples and generates results for each samples that contain likelihood scores for each speaker varying from 0 to 1. Higher scores mean greater possibility that a voice sample belongs to a particular speaker.

\subsubsection{Metric I}

In order to organize over 2000 results in an intuitive manner, we calculate following statistical values from them, respectively for every kind of speech sequences and each speaker:
...
Though our voice samples and results are not exactly intended to be taken by human ears, it's useful to ensure that the desensitized voice samples do not sound like the original ones and the recovered ones are intelligible. In this case, we conduct ABX test, where given original samples of each speaker, participants identify the class of random choices of voice sequences, on numerous subjects.

For this experiment, we built a graphical interface where participants can listen to a particular original speech sequence of each speaker at any point. With the identity of each speaker in mind, participants are instructed to listen to an track that may be any of the results or the original recordings. Then they should make a guess about the identity of this particular sequence. Before the decision, they are able to re-listen to the original voice sequences or repeat the recording to guess.

The type of recording played, which is one variant in the tuple mentioned in our setup, the original speaker and what the participant chose is stored as one record.

\subsubsection{Metric II}
